                ...
                <div class="p-6 text-gray-900 max-w-2xl">
                    <form action="{{ route('tasks.store') }}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" name="title" id="title" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('title') }}" required>
                            @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-5">
                            <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea name="description" id="description" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                            <div>
                                ...
                                @if(Auth::user()->role === \App\Enums\RoleType::EMPLOYEE)
                                    <input type="hidden" name="assigned_to" value="{{ Auth::user()->id }}">
                                    <input type="text" class="block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 text-gray-500 cursor-not-allowed" value="{{ Auth::user()->name }} ({{ str_replace('_', ' ', strtoupper(optional(Auth::user()->role)->value ?? '')) }})" disabled>
                                @else
                                    <select name="assigned_to" id="assigned_to" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                        ...
                                        @foreach($employees as $emp)
                                            <option value="{{ $emp->id }}" {{ old('assigned_to', request('assigned_to')) == $emp->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="responsible_person" id="responsible_person" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($allUsers as $emp)
                                        <option value="{{ $emp->id }}" {{ old('responsible_person') == $emp->id ? 'selected' : '' }}>
                                            {{ $emp->name }} ({{ str_replace('_', ' ', strtoupper(optional($emp->role)->value ?? '')) }})
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-5">
                            ...
                            <input type="date" name="due_date" id="due_date" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('due_date') }}">
                        ...
                        <div class="flex justify-end space-x-3 border-t pt-4">
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Assign Task</button>
                        </div>
                    </form>
                </div>
            </div>
