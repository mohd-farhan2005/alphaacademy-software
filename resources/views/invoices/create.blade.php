                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('invoices.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="student_name" id="student_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required value="{{ old('student_name') }}">
                            @error('student_name')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="batch" id="batch" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                @for($i = 1; $i <= 12; $i++)
                                    <option value="Batch {{ $i }}" {{ old('batch') == "Batch $i" ? 'selected' : '' }}>Batch {{ $i }}</option>
                                @endfor
                            </select>
                            @error('batch')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="price" id="price" step="0.01" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required value="{{ old('price') }}">
                            @error('price')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="service" id="service" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                ...
                                <option value="Fees" {{ old('service') == 'Fees' ? 'selected' : '' }}>Fees</option>
                                <option value="Software" {{ old('service') == 'Software' ? 'selected' : '' }}>Software</option>
                            </select>
                            @error('service')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="flex items-center justify-end mt-4 gap-4">
                            ...
                            <button type="submit" name="save" value="1" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Save</button>
                            <button type="submit" name="save_and_print" value="1" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Save & Print</button>
                        </div>
                    </form>
                </div>
            </div>
